function results = run_AFLCF(seq, res_path, bSaveImage)

% Feature specific parameters
hog_params.cell_size = 4;
hog_params.compressed_dim = 10;
hog_params.nDim = 31;   %%31
...
cnn_params.output_layer = [11];           % Which layers to use
...
params.t_global.cell_size = 8;                  % Feature cell size
...
params.max_image_sample_size = 350^2;   % Maximum area of image samples

% Spatial regularization window_parameters  
params.feature_downsample_ratio = [4, 14]; %= [4, 14]Feature downsample ratio (We found that decreasing the downsampling ratios of CNN layer may benefit the performance)
...
params.temporal_regularization_factor = [15 15]; % The temporal regularization parameters15 15
...
params.scale_step = 1.01;               % The scale factor 1.007
