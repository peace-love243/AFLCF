function res_feat = extract_features(image, pos, scales, features, gparams, extract_info,frame)
...
for feat_ind = 1:num_features
    ...
    if feat.is_cell
        ...
        feature_map(ind:ind+num_blocks-1) = feat.getFeature(img_samples{img_sample_ind}, feat.fparams, gparams,frame);
        CNN_ind = ind;
    else
        num_blocks = 1;
        feature_map{ind} = feat.getFeature(img_samples{img_sample_ind}, feat.fparams, gparams,frame);
    end
...
if(CNN_ind ~=-1)
    if(num_features == 2)
        res_feat{1} = cat(3,feature_map{1});    
% if(num_features == 3)
